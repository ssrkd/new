...
from backend.llm_client import call_llm_json_for_news
...
4. В массив "tags" ОБЯЗАТЕЛЬНО включи одну или несколько из базовых категорий: казахстан, мир, дипломатия, экономика, безопасность. 
   ВНИМАНИЕ ПО ТЕГУ "казахстан": Ставь этот тег ТОЛЬКО если новость напрямую касается событий в Казахстане или его граждан/политиков. Если новость про другую страну (например, Россию) и Казахстан лишь упомянут вскользь — НЕ ставь тег "казахстан", ставь тег страны, о которой новость (например, "россия"). Плюс любые другие релевантные теги.
...
async def process_article(raw: dict) -> bool:
    ...
    try:
        result = await call_llm_json_for_news(messages, max_tokens=2000)
